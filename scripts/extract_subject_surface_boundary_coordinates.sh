...
echo
echo  "usage: extract_subject_surface_boundary_coordinates.sh <subject's freesurfer directory>"
echo
echo ""   
...
echo ""
echo "* Requires that R is installed *"
echo ""   
exit 1
fi
...
echo "indices <- read.table(\"tmpLabel_\")" >> selectPoints_l.R
...
for x in `../label/rh.*.label`
do
   # first line removes the two first lines because they contain header info,
   awk '//{if (NR>2){print $0}}' < ${x} > tmpLabel_r
   octave selectPoints_r.R
   BASE=`basename $x .label`
